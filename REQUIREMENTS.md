...
- v2 主题索引允许持久化有界且唯一的 `installedPresetPacks`。未安装主题包应在一次 store-owned 事务内追加 ready 主题、复制图片并提交 pack 标记，不改变已有主题、revision、选择项、last-known-good、暂停状态或 checkpoint；失败必须恢复旧索引并清理已暂存图片。已标记主题包不再读取或补种，用户删除其中主题后不得在重启时复活。
...
24. 当前内置图片主题包包含 13 个一图一主题的 ready 记录；全新主题库得到原有 2 个基础主题加 13 个图片主题，既有 v1/v2 主题库只追加一次。重复启动不产生重复项，删除任一图片主题后不会重建；catalog、任一图片或持久化步骤失败时主题库保持原状。安装包中的 catalog 与 13 张图片必须位于受 ASAR 完整性保护的固定资源路径并逐项通过 SHA-256 校验。
