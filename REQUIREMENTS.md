...
- 双击已发送用户消息后的编辑表单复用“输入框与我的消息”的背景及 alpha、“输入文字”的正文及光标、发送按钮的 accent/accentText。取消按钮使用 secondary/line 与透明背景；输入框配方关闭或高级模式时不强制覆盖表面。选择器限定在用户消息锚点内已核对的富文本表单，后挂载立即生效，退出编辑或复用表单后撤销映射；保留文字发送按钮、加载/禁用/聚焦状态及提及菜单，不新增配置或将其作为普通弹窗处理。
- 新启动会话首次验证成功后、注入前若页面短暂重建，可在额外 15 秒宽限内重试只读就绪检查；每次必须重验同一进程、nonce、监听者、Browser ID 和 target，身份不匹配立即失败。持续不兼容仍须拦截，等待中不得注入或自动重启。失败提示须区分 CDP 不可用、身份不符和页面不兼容，不得统一提示更新主题适配。
...
31. “输入文字”必须作为独立结构化颜色，作用于首页与对话 composer、已发送用户消息编辑框的已输入正文和光标；旧主题缺少该字段时回落到通用正文色。占位文字继续消费 muted，输入框底部工具栏和首页“选择项目”rail 继续消费 secondary，三者不得互相覆盖。LIVE PREVIEW 必须同时展示并可反向定位输入正文、占位文字和工具栏文字。
