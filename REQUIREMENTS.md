...
- 当前 IPC 为 `v: 2`；`rendererReady` 使用固定 `v: 1` bootstrap 并返回主进程版本/协议，专门识别覆盖安装后仍驻留的旧主进程。其余请求版本不一致时必须返回明确的不兼容错误。该边界不改变 `../old/` 或主题 ZIP 三件套兼容契约。
...
22. 普通编辑区只显示一个“保存主题”操作，并自动保持 patch-before-commit 的 revision 顺序；高级 JSON 的独立校验边界不变。
