...
- 本地主题库支持无损的当前主题 ZIP，以及显式面向 v1.0.x/v1.1.x 的旧版兼容 ZIP；兼容导出会移除三个新颜色字段，并拒绝旧版无法读取的高级 CSS。
...
从 [GitHub Releases](https://github.com/Zhqiankun/codexDream/releases/latest) 下载 `v1.2.0`：

- `CodexStyle-1.2.0-x64.exe` — Windows 安装程序。
- `CodexStyle-1.2.0-x64.zip` — 免安装压缩包。
- `SHA256SUMS.txt` — 发布包与更新元数据的 SHA-256 校验值。

当前发布包未进行代码签名，Windows SmartScreen 可能显示“未知发布者”提示。运行前请先核对 SHA-256 校验值。

`v1.1.0` 首次加入应用内更新器。`v1.0.1` 及更早版本需要手动安装一次 `v1.1.0` 或更高版本，直接安装当前 `v1.2.0` 即可；之后正式安装版可以在应用内升级。ZIP 便携版仍需手动更新。
...
需要保留全部当前字段时使用**导出主题 ZIP**；仅在分享给 CodexStyle v1.0.x 或 v1.1.x 时使用**导出旧版兼容 ZIP**。未编辑的正式导入包可以原样导出，一旦编辑便不能再重建原始正式包。
