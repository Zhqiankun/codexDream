...
从 [GitHub Releases](https://github.com/Zhqiankun/codexDream/releases/latest) 下载 `v1.3.9`：

- `CodexStyle-1.3.9-x64.exe` — Windows 安装程序。
- `CodexStyle-1.3.9-x64.zip` — 免安装压缩包。
- `SHA256SUMS.txt` — 发布包与更新元数据的 SHA-256 校验值。

当前发布包未进行代码签名，Windows SmartScreen 可能显示“未知发布者”提示。运行前请先核对 SHA-256 校验值。

`v1.3.9` 修复 Codex `26.825.4187.0` 中当前会话标签背景与首页标题颜色不生效的问题，并为首页四张快捷卡片增加独立颜色/图片背景。卡片图片只在本机压缩为有界 WebP 缩略图，并随现有三件套主题 ZIP 保存。直接覆盖原安装即可，无需卸载或重启电脑；选择新主题后重启由 CodexStyle 管理的 Codex 会话即可看到效果。
