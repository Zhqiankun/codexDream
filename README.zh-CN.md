...
- 本地主题库支持无损的当前主题 ZIP；历史十色或十二色 ZIP 仍可导入，但不再提供有损的旧版兼容导出。
...
从 [GitHub Releases](https://github.com/Zhqiankun/codexDream/releases/latest) 下载 `v1.3.6`：

- `CodexStyle-1.3.6-x64.exe` — Windows 安装程序。
- `CodexStyle-1.3.6-x64.zip` — 免安装压缩包。
- `SHA256SUMS.txt` — 发布包与更新元数据的 SHA-256 校验值。

当前发布包未进行代码签名，Windows SmartScreen 可能显示“未知发布者”提示。运行前请先核对 SHA-256 校验值。

`v1.3.6` 将 13 套壁纸预设统一调整为页面、面板和侧栏 20% 透明度及边框 10%，对未编辑的 v1 预设安全原位迁移；左侧主题库新增背景缩略图，启动主题选择移到编辑器上方，并移除旧版兼容 ZIP 导出。用户改过或删过的预设保持不动。直接覆盖原安装即可，无需卸载；能够正常打开的 v1.3.1 及以上安装版可使用应用内更新。ZIP 便携版仍需手动更新。
